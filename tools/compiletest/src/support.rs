...
pub fn meta_path(deps: &Path, name: &str) -> PathBuf {
  deps.join(format!("lib{}", name)).with_extension("rmeta")
}

pub fn llvm_path(deps: &Path, name: &str) -> PathBuf {
  deps.join(name).with_extension("ll")
}
