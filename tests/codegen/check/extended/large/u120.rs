...
pub fn ucmp(a: uint, b: uint) -> ::core::cmp::Ordering {
  // CHECK: %[[C:.+]] = load i120, ptr %[[A]], align 1
  // CHECK: %[[D:.+]] = load i120, ptr %[[B]], align 1
  // CHECK: %[[E:.+]] = tail call noundef range(i8 -1, 2) i8 @llvm.ucmp.i8.i120(i120 %[[C]], i120 %[[D]])
  // CHECK: ret i8 %[[E]]
...
pub fn scmp(a: int, b: int) -> ::core::cmp::Ordering {
  // CHECK: %[[C:.+]] = load i120, ptr %[[A]], align 1
  // CHECK: %[[D:.+]] = load i120, ptr %[[B]], align 1
  // CHECK: %[[E:.+]] = tail call noundef range(i8 -1, 2) i8 @llvm.scmp.i8.i120(i120 %[[C]], i120 %[[D]])
  // CHECK: ret i8 %[[E]]
...
pub fn swap1(a: uint) -> uint {
  // CHECK: %[[C:.+]] = load i120, ptr %[[A]], align 1
  // CHECK: %[[D:.+]] = tail call i120 @llvm.bitreverse.i120(i120 %[[C]])
  // CHECK: store i120 %[[D]], ptr %[[B]], align 1
  // CHECK: ret void
...
// CHECK-LABEL: define noundef range(i32 0, 121) i32 @ctpop
// CHECK-SAME: (ptr {{.*}} %[[A:.+]])
#[unsafe(no_mangle)]
pub fn ctpop(a: uint) -> u32 {
  // CHECK: %[[B:.+]] = load i120, ptr %[[A]], align 1
  // CHECK: %[[C:.+]] = tail call range(i120 0, 121) i120 @llvm.ctpop.i120(i120 %[[B]])
  // CHECK: %[[D:.+]] = trunc nuw nsw i120 %[[C]] to i32
  // CHECK: ret i32 %[[D]]
  exint::backend::ctpop::<_, N>(a)
}

// CHECK-LABEL: define noundef range(i32 0, 121) i32 @ctlz
// CHECK-SAME: (ptr {{.*}} %[[A:.+]])
#[unsafe(no_mangle)]
pub fn ctlz(a: uint) -> u32 {
  // CHECK: %[[B:.+]] = load i120, ptr %[[A]], align 1
  // CHECK: %[[C:.+]] = tail call range(i120 0, 121) i120 @llvm.ctlz.i120(i120 %[[B]], i1 false)
  // CHECK: %[[D:.+]] = trunc nuw nsw i120 %[[C]] to i32
  // CHECK: ret i32 %[[D]]
  exint::backend::ctlz::<_, N>(a)
}

// CHECK-LABEL: define noundef range(i32 0, 121) i32 @cttz
// CHECK-SAME: (ptr {{.*}} %[[A:.+]])
#[unsafe(no_mangle)]
...
// CHECK-LABEL: define noundef range(i32 0, 120) i32 @ctlz_nonzero
// CHECK-SAME: (ptr {{.*}} %[[A:.+]])
#[unsafe(no_mangle)]
pub fn ctlz_nonzero(a: uint) -> u32 {
  // CHECK: %[[B:.+]] = load i120, ptr %[[A]], align 1
  // CHECK: %[[C:.+]] = tail call range(i120 0, 121) i120 @llvm.ctlz.i120(i120 %[[B]], i1 true)
  // CHECK: %[[D:.+]] = trunc nuw nsw i120 %[[C]] to i32
  // CHECK: ret i32 %[[D]]
  unsafe { exint::backend::ctlz_nonzero::<_, N>(a) }
}

// CHECK-LABEL: define noundef range(i32 0, 120) i32 @cttz_nonzero
// CHECK-SAME: (ptr {{.*}} %[[A:.+]])
#[unsafe(no_mangle)]
...
pub fn overflowing_uadd(a: uint, b: uint) -> (uint, bool) {
  ...
  // CHECK: %[[G:.+]] = icmp ult i120 %[[F]], %[[D]]
  // CHECK: %[[H:.+]] = zext i1 %[[G]] to i8
  // CHECK: store i120 %[[F]], ptr %[[C]], align 1
  // CHECK: %[[I:.+]] = getelementptr inbounds nuw i8, ptr %[[C]], i64 15
...
pub fn overflowing_usub(a: uint, b: uint) -> (uint, bool) {
  // CHECK: %[[D:.+]] = load i120, ptr %[[A]], align 1
  // CHECK: %[[E:.+]] = load i120, ptr %[[B]], align 1
  // CHECK: %[[F:.+]] = icmp ult i120 %[[D]], %[[E]]
  // CHECK: %[[G:.+]] = sub i120 %[[D]], %[[E]]
  // CHECK: %[[H:.+]] = zext i1 %[[F]] to i8
  // CHECK: store i120 %[[G]], ptr %[[C]], align 1
  // CHECK: %[[I:.+]] = getelementptr inbounds nuw i8, ptr %[[C]], i64 15
  // CHECK: store i8 %[[H]], ptr %[[I]], align 1
  // CHECK: ret void
...
pub fn overflowing_umul(a: uint, b: uint) -> (uint, bool) {
  // CHECK: %[[D:.+]] = load i120, ptr %[[A]], align 1
  // CHECK: %[[E:.+]] = load i120, ptr %[[B]], align 1
  // CHECK: %[[F:.+]] = freeze i120 %[[D]]
  // CHECK: %[[G:.+]] = tail call { i120, i1 } @llvm.umul.with.overflow.i120(i120 %[[E]], i120 %[[F]])
  // CHECK: %[[H:.+]] = extractvalue { i120, i1 } %[[G]], 1
  // CHECK: %[[I:.+]] = zext i1 %[[H]] to i8
  // CHECK: %[[J:.+]] = extractvalue { i120, i1 } %[[G]], 0
  // CHECK: store i120 %[[J]], ptr %[[C]], align 1
  // CHECK: %[[K:.+]] = getelementptr inbounds nuw i8, ptr %[[C]], i64 15
...
pub fn saturating_usub(a: uint, b: uint) -> uint {
  // CHECK: %[[D:.+]] = load i120, ptr %[[A]], align 1
  // CHECK: %[[E:.+]] = load i120, ptr %[[B]], align 1
  // CHECK: %[[F:.+]] = tail call i120 @llvm.usub.sat.i120(i120 %[[D]], i120 %[[E]])
  // CHECK: store i120 %[[F]], ptr %[[C]], align 1
  // CHECK: ret void
...
pub fn saturating_ssub(a: int, b: int) -> int {
  // CHECK: %[[D:.+]] = load i120, ptr %[[A]], align 1
  // CHECK: %[[E:.+]] = load i120, ptr %[[B]], align 1
  // CHECK: %[[F:.+]] = tail call i120 @llvm.ssub.sat.i120(i120 %[[D]], i120 %[[E]])
  // CHECK: store i120 %[[F]], ptr %[[C]], align 1
  // CHECK: ret void
...
pub fn unchecked_srem(a: int, b: int) -> int {
  ...
  // CHECK: %[[I:.+]] = trunc nsw i128 %[[H]] to i120
  // CHECK: store i120 %[[I]], ptr %[[C]], align 1
  // CHECK: ret void
