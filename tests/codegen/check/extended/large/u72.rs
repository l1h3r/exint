...
pub fn ucmp(a: uint, b: uint) -> ::core::cmp::Ordering {
  // CHECK: %[[C:.+]] = load i72, ptr %[[A]], align 1
  // CHECK: %[[D:.+]] = load i72, ptr %[[B]], align 1
  // CHECK: %[[E:.+]] = tail call noundef range(i8 -1, 2) i8 @llvm.ucmp.i8.i72(i72 %[[C]], i72 %[[D]])
  // CHECK: ret i8 %[[E]]
...
pub fn scmp(a: int, b: int) -> ::core::cmp::Ordering {
  // CHECK: %[[C:.+]] = load i72, ptr %[[A]], align 1
  // CHECK: %[[D:.+]] = load i72, ptr %[[B]], align 1
  // CHECK: %[[E:.+]] = tail call noundef range(i8 -1, 2) i8 @llvm.scmp.i8.i72(i72 %[[C]], i72 %[[D]])
  // CHECK: ret i8 %[[E]]
...
pub fn swap1(a: uint) -> uint {
  // CHECK: %[[C:.+]] = load i72, ptr %[[A]], align 1
  // CHECK: %[[D:.+]] = tail call i72 @llvm.bitreverse.i72(i72 %[[C]])
  // CHECK: store i72 %[[D]], ptr %[[B]], align 1
  // CHECK: ret void
...
// CHECK-LABEL: define noundef range(i32 0, 73) i32 @ctpop
// CHECK-SAME: (ptr {{.*}} %[[A:.+]])
#[unsafe(no_mangle)]
pub fn ctpop(a: uint) -> u32 {
  // CHECK: %[[B:.+]] = load i72, ptr %[[A]], align 1
  // CHECK: %[[C:.+]] = tail call range(i72 0, 73) i72 @llvm.ctpop.i72(i72 %[[B]])
  // CHECK: %[[D:.+]] = trunc nuw nsw i72 %[[C]] to i32
  // CHECK: ret i32 %[[D]]
  exint::backend::ctpop::<_, N>(a)
}

// CHECK-LABEL: define noundef range(i32 0, 73) i32 @ctlz
// CHECK-SAME: (ptr {{.*}} %[[A:.+]])
#[unsafe(no_mangle)]
pub fn ctlz(a: uint) -> u32 {
  // CHECK: %[[B:.+]] = load i72, ptr %[[A]], align 1
  // CHECK: %[[C:.+]] = tail call range(i72 0, 73) i72 @llvm.ctlz.i72(i72 %[[B]], i1 false)
  // CHECK: %[[D:.+]] = trunc nuw nsw i72 %[[C]] to i32
  // CHECK: ret i32 %[[D]]
  exint::backend::ctlz::<_, N>(a)
}

// CHECK-LABEL: define noundef range(i32 0, 73) i32 @cttz
// CHECK-SAME: (ptr {{.*}} %[[A:.+]])
#[unsafe(no_mangle)]
...
// CHECK-LABEL: define noundef range(i32 0, 72) i32 @ctlz_nonzero
// CHECK-SAME: (ptr {{.*}} %[[A:.+]])
#[unsafe(no_mangle)]
pub fn ctlz_nonzero(a: uint) -> u32 {
  // CHECK: %[[B:.+]] = load i72, ptr %[[A]], align 1
  // CHECK: %[[C:.+]] = tail call range(i72 0, 73) i72 @llvm.ctlz.i72(i72 %[[B]], i1 true)
  // CHECK: %[[D:.+]] = trunc nuw nsw i72 %[[C]] to i32
  // CHECK: ret i32 %[[D]]
  unsafe { exint::backend::ctlz_nonzero::<_, N>(a) }
}

// CHECK-LABEL: define noundef range(i32 0, 72) i32 @cttz_nonzero
// CHECK-SAME: (ptr {{.*}} %[[A:.+]])
#[unsafe(no_mangle)]
...
pub fn overflowing_uadd(a: uint, b: uint) -> (uint, bool) {
  ...
  // CHECK: %[[E:.+]] = load i72, ptr %[[B]], align 1
  // CHECK: %[[F:.+]] = add i72 %[[E]], %[[D]]
  // CHECK: %[[G:.+]] = icmp ult i72 %[[F]], %[[D]]
  // CHECK: %[[H:.+]] = zext i1 %[[G]] to i8
  // CHECK: store i72 %[[F]], ptr %[[C]], align 1
  // CHECK: %[[I:.+]] = getelementptr inbounds nuw i8, ptr %[[C]], i64 9
...
pub fn overflowing_usub(a: uint, b: uint) -> (uint, bool) {
  // CHECK: %[[D:.+]] = load i72, ptr %[[A]], align 1
  // CHECK: %[[E:.+]] = load i72, ptr %[[B]], align 1
  // CHECK: %[[F:.+]] = icmp ult i72 %[[D]], %[[E]]
  // CHECK: %[[G:.+]] = sub i72 %[[D]], %[[E]]
  // CHECK: %[[H:.+]] = zext i1 %[[F]] to i8
  // CHECK: store i72 %[[G]], ptr %[[C]], align 1
  // CHECK: %[[I:.+]] = getelementptr inbounds nuw i8, ptr %[[C]], i64 9
  // CHECK: store i8 %[[H]], ptr %[[I]], align 1
  // CHECK: ret void
...
pub fn overflowing_umul(a: uint, b: uint) -> (uint, bool) {
  // CHECK: %[[D:.+]] = load i72, ptr %[[A]], align 1
  // CHECK: %[[E:.+]] = load i72, ptr %[[B]], align 1
  // CHECK: %[[F:.+]] = freeze i72 %[[D]]
  // CHECK: %[[G:.+]] = tail call { i72, i1 } @llvm.umul.with.overflow.i72(i72 %[[E]], i72 %[[F]])
  // CHECK: %[[H:.+]] = extractvalue { i72, i1 } %[[G]], 1
  // CHECK: %[[I:.+]] = zext i1 %[[H]] to i8
  // CHECK: %[[J:.+]] = extractvalue { i72, i1 } %[[G]], 0
  // CHECK: store i72 %[[J]], ptr %[[C]], align 1
  // CHECK: %[[K:.+]] = getelementptr inbounds nuw i8, ptr %[[C]], i64 9
...
pub fn saturating_usub(a: uint, b: uint) -> uint {
  // CHECK: %[[D:.+]] = load i72, ptr %[[A]], align 1
  // CHECK: %[[E:.+]] = load i72, ptr %[[B]], align 1
  // CHECK: %[[F:.+]] = tail call i72 @llvm.usub.sat.i72(i72 %[[D]], i72 %[[E]])
  // CHECK: store i72 %[[F]], ptr %[[C]], align 1
  // CHECK: ret void
...
pub fn saturating_ssub(a: int, b: int) -> int {
  // CHECK: %[[D:.+]] = load i72, ptr %[[A]], align 1
  // CHECK: %[[E:.+]] = load i72, ptr %[[B]], align 1
  // CHECK: %[[F:.+]] = tail call i72 @llvm.ssub.sat.i72(i72 %[[D]], i72 %[[E]])
  // CHECK: store i72 %[[F]], ptr %[[C]], align 1
  // CHECK: ret void
...
pub fn unchecked_srem(a: int, b: int) -> int {
  ...
  // CHECK: %[[I:.+]] = trunc nsw i128 %[[H]] to i72
  // CHECK: store i72 %[[I]], ptr %[[C]], align 1
  // CHECK: ret void
