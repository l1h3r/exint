...
pub fn int_1(a: int<1>, b: u32) -> int<1> {
  ...
  // CHECK: %[[reg_b:.*]] = tail call i8 @llvm.fshl.i8(i8 %{{.*}}, i8 %{{.*}}, i8 %[[reg_a]])
  // CHECK: ret i8 %[[reg_b]]
  a.rotate_left(b)
}
...
pub fn int_2(a: int<2>, b: u32) -> int<2> {
  ...
  // CHECK: %[[reg_b:.*]] = tail call i16 @llvm.fshl.i16(i16 %{{.*}}, i16 %{{.*}}, i16 %[[reg_a]])
  // CHECK: ret i16 %[[reg_b]]
  a.rotate_left(b)
}
...
pub fn int_4(a: int<4>, b: u32) -> int<4> {
  // CHECK: %[[reg:.*]] = tail call i32 @llvm.fshl.i32(i32 %{{.*}}, i32 %{{.*}}, i32 %{{.*}})
  // CHECK: ret i32 %[[reg]]
  a.rotate_left(b)
}
...
pub fn int_8(a: int<8>, b: u32) -> int<8> {
  ...
  // CHECK: %[[reg_b:.*]] = tail call i64 @llvm.fshl.i64(i64 %{{.*}}, i64 %{{.*}}, i64 %[[reg_a]])
  // CHECK: ret i64 %[[reg_b]]
  a.rotate_left(b)
}
...
pub fn int_16(a: int<16>, b: u32) -> int<16> {
  ...
  // CHECK: %[[reg_c:.*]] = tail call i128 @llvm.fshl.i128(i128 %[[reg_a]], i128 %[[reg_a]], i128 %[[reg_b]])
