...
pub fn uint_1(a: uint<1>, b: uint<1>) -> (uint<1>, bool) {
  // CHECK: %[[C:.+]] = tail call { i8, i1 } @llvm.uadd.with.overflow.i8(i8 %[[A]], i8 %[[B]])
...
pub fn uint_2(a: uint<2>, b: uint<2>) -> (uint<2>, bool) {
  // CHECK: %[[C:.+]] = tail call { i16, i1 } @llvm.uadd.with.overflow.i16(i16 %[[A]], i16 %[[B]])
...
pub fn uint_4(a: uint<4>, b: uint<4>) -> (uint<4>, bool) {
  // CHECK: %[[C:.+]] = tail call { i32, i1 } @llvm.uadd.with.overflow.i32(i32 %[[A]], i32 %[[B]])
...
pub fn uint_8(a: uint<8>, b: uint<8>) -> (uint<8>, bool) {
  // CHECK: %[[D:.+]] = tail call { i64, i1 } @llvm.uadd.with.overflow.i64(i64 %[[A]], i64 %[[B]])
...
pub fn uint_16(a: uint<16>, b: uint<16>) -> (uint<16>, bool) {
  ...
  // CHECK: %[[F:.+]] = tail call { i128, i1 } @llvm.uadd.with.overflow.i128(i128 %[[D]], i128 %[[E]])
